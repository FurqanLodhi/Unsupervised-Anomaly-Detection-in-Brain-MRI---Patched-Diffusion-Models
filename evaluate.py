...
class Evaluator:
    ...
    def evaluate(self):
        ...
        for dataset_key in self.test_data_dict.keys():
            ...
            for idx, data in enumerate(dataset):
                ...
                for i in range(nr_batches):
                    ...
                    pred_mask = ano_map_i.copy()

                    pred_mask[pred_mask > 0.25] = 1
                    pred_mask[pred_mask <= 0.25] = 0

                    flat_pred_masks = pred_mask.flatten()

                    flat_ano_maps = ano_map_i.flatten()
                    flat_masks = mask_i.flatten()
                    ...
                    roc_auc = roc_auc_score(flat_masks, flat_ano_maps)
                    test_metrics['AUROC'].append(roc_auc)

                    precision, recall, _ = precision_recall_curve(flat_masks, flat_pred_masks)
                    pr_auc = auc(recall, precision)
                    test_metrics['AUPRC'].append(pr_auc)

                    intersection = np.sum(flat_pred_masks * flat_masks)
                    dice_score = (2. * intersection) / (np.sum(flat_pred_masks) + np.sum(flat_masks))
                    test_metrics['DICE'].append(dice_score)
                    ...
                    fig, axarr = plt.subplots(1, len(elements), figsize=(len(elements) * 4, 4))
                    for idx_arr in range(len(axarr)):
                        cmap = 'plasma' if idx_arr in [2, 4, 5] else 'gray'
                        axarr[idx_arr].imshow(elements[idx_arr], cmap=cmap, vmin=0, vmax=v_maxs[idx_arr])
                        axarr[idx_arr].set_title(titles[idx_arr])
                        axarr[idx_arr].axis('off')
                    # plt.show()
                    ...
                    # fig.savefig("./" + dataset_key + '_{}'.format(i), dpi=100)
                    plt.close(fig)
        ...
        return metrics, fig_bps, all_figures
